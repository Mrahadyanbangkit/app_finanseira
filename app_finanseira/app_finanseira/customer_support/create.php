...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO `tb_customer_support` VALUES (NULL, '$tanggal', '$user_id', '$jenis_pengaduan', '$deskripsi')";

    // Melakukan query ke database
    if (mysqli_query($koneksi, $query)) {
        echo '<script>alert("Data berhasil disimpan.");window.location.href="' . $base_url . 'customer_support"</script>';
    } else {
                                ...
                                <div class="card-body">
                                    <form action="" method="post">
                                        <div class="form-group">
                                            ...
                                            <input type="date" class="form-control form-control-sm" name="tanggal" id="tanggal" placeholder="Tanggal" min="1" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="nama_lengkap" class="col-form-label col-form-label-sm">Nama Pembuat</label>
                                            <input type="hidden" name="user_id" id="user_id" value="<?= $_SESSION['user_id'] ?>">
                                            <input type="text" class="form-control form-control-sm bg-transparent" name="nama_lengkap" id="nama_lengkap" placeholder="Nama Pembuat" value="<?= $_SESSION['nama_lengkap'] ?>" autocomplete="off" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label for="jenis_pengaduan" class="col-form-label col-form-label-sm">Jenis Pengaduan</label>
                                            <input type="text" name="jenis_pengaduan" id="jenis_pengaduan" class="form-control form-control-sm" placeholder="Jenis Pengaduan" autocomplete="off" maxlength="100" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="deskripsi" class="col-form-label col-form-label-sm">Deskripsi</label>
                                            <textarea name="deskripsi" id="deskripsi" cols="30" rows="4" class="form-control form-control-sm" placeholder="Deskripsi" autocomplete="off" required></textarea>
                                        </div>

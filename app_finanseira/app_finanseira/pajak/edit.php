...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT * FROM `tb_pajak` WHERE `pajak_id` = '$id'";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
        ...
        $query = "UPDATE `tb_pajak` SET `tanggal`='$tanggal', `gaji`='$gaji',`nama_lengkap`='$nama_lengkap',`jenis_pajak`='$jenis_pajak',`npwp`='$npwp',`persentase`='$persentase',`jumlah`='$jumlah',`deskripsi`='$jumlah',`tipe`='$tipe' WHERE `pajak_id`='$id'";

        // Melakukan query ke database
        if (mysqli_query($koneksi, $query)) {
            echo '<script>alert("Data berhasil diperbaharui.");window.location.href="' . $base_url . 'pajak"</script>';
        } else {
                        ...
                        <div class="col-lg-12">
                            <div class="card mb-4">
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    ...
                                    <button type="button" class="btn btn-secondary btn-sm" onclick="window.location.href='<?= $base_url ?>pajak'">Kembali</button>
                                </div>
                                <div class="card-body">
                                    <form action="" method="post">
                                        ...
                                        <div class="form-group">
                                            <label for="gaji" class="col-form-label col-form-label-sm">Masukan Gaji</label>
                                            <input type="number" class="form-control form-control-sm" name="gaji" id="gaji" placeholder="Masukan Gaji" min="1" value="<?= $res['gaji'] ?>" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="nama_lengkap" class="col-form-label col-form-label-sm">Nama Lengkap</label>
                                            <input type="text" class="form-control form-control-sm" name="nama_lengkap" id="nama_lengkap" placeholder="Nama Lengkap" value="<?= $res['nama_lengkap'] ?>" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="jenis_pajak" class="col-form-label col-form-label-sm">Jenis Pajak</label>
                                            <select name="jenis_pajak" id="jenis_pajak" class="form-control form-control-sm" required>
                                                <option value="">--- Choose ---</option>
                                                <option value="BPJS Ketenagakerjaan" <?= $res['jenis_pajak'] === 'PPH' ? 'selected' : '' ?>>PPH</option>
                                                <option value="Pajak Penghasilan" <?= $res['jenis_pajak'] === 'PPN' ? 'selected' : '' ?>>PPN</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label for="npwp" class="col-form-label col-form-label-sm">NPWP</label>
                                            <input type="number" class="form-control form-control-sm" name="npwp" id="npwp" placeholder="NPWP" min="1" value="<?= $res['npwp'] ?>" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="persentase" class="col-form-label col-form-label-sm">Persentase %</label>
                                            <input type="number" class="form-control form-control-sm" name="persentase" id="persentase" placeholder="Persentase %" min="1" max="100" value="<?= $res['persentase'] ?>" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="jumlah" class="col-form-label col-form-label-sm">Jumlah</label>
                                            <input type="text" class="form-control form-control-sm bg-transparent" name="jumlah" id="jumlah" placeholder="Jumlah" value="<?= $res['jumlah'] ?>" autocomplete="off" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label for="deskripsi" class="col-form-label col-form-label-sm">Deskripsi</label>
                                            <input type="text" class="form-control form-control-sm" name="deskripsi" id="deskripsi" placeholder="Deskripsi" value="<?= $res['deskripsi'] ?>" autocomplete="off">
                                        </div>
                                        <div class="form-group">

...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $id = $_GET['id'];
    $query = "SELECT * FROM `tb_buyyer` WHERE `buy_id` = '$id'";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
        ...
        $query = "UPDATE `tb_buyyer` SET `tanggal`='$tanggal',`jenis_pembelian`='$jenis_pembelian',`deskripsi`='$deskripsi',`jumlah`='$jumlah',`tipe`='$tipe' WHERE `buy_id`='$id'";

        // Melakukan query ke database
        if (mysqli_query($koneksi, $query)) {
            echo '<script>alert("Data berhasil diperbaharui.");window.location.href="' . $base_url . 'buyyer"</script>';
        } else {
                        ...
                        <div class="col-lg-12">
                            <div class="card mb-4">
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    ...
                                    <button type="button" class="btn btn-secondary btn-sm" onclick="window.location.href='<?= $base_url ?>buyyer'">Kembali</button>
                                </div>
                                <div class="card-body">
                                    <form action="" method="post">
                                        ...
                                        <FORM METHOD=POST action="<?php echo $_SERVER["PHP_SELF"];?>"> 
                                            <label for="jenis_pembelian" class="col-form-label col-form-label-sm">Jenis Pembelian</label>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control form-control-sm" name="deskripsi" id="deskripsi" placeholder="Deskripsi" value="<?= $res['deskripsi'] ?>" autocomplete="off" required>
                                        </div>
                                        <div class="form-group">
